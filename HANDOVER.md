...
| `pages/campaign.html?id=…` | **Campaign page** — stage track, *Next up* strip, run-date timeline, deliverables ring, and the tabs Overview · Selection · Deliverables · Documents · Activity. Deliverables has a queue over the per-creator tables (needs our review · with the client · with creators · overdue · due this week · posted — `campaignModel.waitingOn` reads the court off status and the client's word), a fold-out review card per row (status track, client switch, link, caption, notes, one decision button), and *Plan deliverables* which creates one per confirmed channel with nothing planned, due dates spread across the run (`campaignModel.planDeliverables`, `campaignStore.addDeliverables`). Documents holds the campaign's files — the brief, decks, the IO — attached on the form's first step or dropped on the tab, kept on the record as data URLs up to 1.5 MB (bigger ones by name only, since this prototype lives in localStorage), opened from that data; the influencer document table below it is still a designed placeholder. C toggles a rail of every campaign, [ and ] step between them; opening a campaign from the list is a cross-page view transition (the card grows into the hero). |
...
shared/           generated data + small runtime helpers
  ...
  swing-modal.js         the fold-out card modal (flies out of what you clicked) and the influencer
                         ...
                         profiles, batches, the campaign form, the send sheet and delete
                         confirms (swingModal.confirm). Add by hand is not a popup: it sends
                         you to the influencers page with ?for=<campaign>&band=plat/tier,
                         which adds the ticked people to the roster and comes back
